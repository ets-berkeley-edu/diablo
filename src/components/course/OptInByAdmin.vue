  ...
  <div v-if="currentUser.isAdmin" class="mt-2">
    <h3 v-if="course.instructors.length" id="instructors-header">
      <span v-if="course.hasOptedIn && course.scheduled">
        {{ course.instructors.length === 1 ? (course.instructors[0].uid === currentUser.uid ? 'You' : 'The instructor below') : 'The instructors below' }}
    ...
    <div v-if="!course.instructors.length" class="mt-2 text-medium-emphasis">
      <ToggleOptIn
